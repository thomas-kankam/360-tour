...
class OperatorToursServiceApi {
  ...
  async getTour(token, slug) {
    const result = await this.request("GET", `/admin/listings/${encodeURIComponent(slug)}`, { token });
  ...
  async createTour(token, payload) {
    const result = await this.request("POST", "/admin/listings", { token, body: payload, dedupe: false });
    if (!result.ok) return { ...result, tour: null };
  ...
  async updateTour(token, slug, payload) {
    const result = await this.request("PUT", `/admin/listings/${encodeURIComponent(slug)}`, { token, body: payload, dedupe: false });
    if (!result.ok) return { ...result, tour: null };
