...
export default function OperatorDashboardPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {user?.isVerified ? (
        <div className="inline-flex items-center gap-2 rounded-full border border-brand-primary/20 bg-brand-primary/8 px-4 py-2 text-sm font-semibold text-brand-primary">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Live listings" value={published} sub={`${drafts} draft${drafts === 1 ? "" : "s"} in progress`} accent="bg-brand-primary" />
      ...
      <div className="grid gap-6 lg:grid-cols-5">
        <section className="lg:col-span-3 rounded-2xl border border-brand-border/60 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            ...
            <Link to={ROUTES.operator.tours} className="text-sm font-semibold text-brand-primary hover:underline">View all</Link>
          </div>
          {tours.length === 0 ? (
            ...
            <ul className="mt-5 divide-y divide-brand-border/50">
              {tours.slice(0, 5).map((tour) => (
                <li key={tour.slug || tour.name} className="flex flex-wrap items-center gap-4 py-4 first:pt-0">
                  ...
                  <Link to={ROUTES.operator.tourDetail(tour.slug)} className="text-sm font-semibold text-brand-primary hover:underline">Manage</Link>
                </li>
              ))}
            </ul>
          )}
