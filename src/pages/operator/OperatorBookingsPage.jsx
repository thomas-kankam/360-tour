...
function BookingLookupResult({ booking, onClear, onRecordPayment }) {
  ...
  return (
    <motion.div
      ...
      className="overflow-hidden rounded-2xl border border-brand-primary/25 bg-white shadow-[0_16px_48px_-20px_rgba(0,107,63,0.28)]"
    >
      <div className="relative">
        {booking.tour?.image ? (
          ...
          <div className="border-b border-brand-border/40 bg-brand-primary/5 px-5 py-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="font-mono text-xs font-bold text-brand-primary">{booking.bookingRef}</p>
      ...
      <div className="grid gap-5 p-5 sm:grid-cols-2 sm:p-6">
        <div className="rounded-xl border border-brand-border/50 bg-brand-cream/40 p-4">
          ...
          <p className="mt-1 text-lg font-bold text-brand-primary">{booking.amountLabel}</p>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Link to={ROUTES.operator.tours} className="text-sm font-semibold text-brand-primary hover:text-brand-primary-dark">
          Manage listings →
        </Link>
      </div>

      <section className="overflow-hidden rounded-2xl border border-brand-border/60 bg-white shadow-sm">
        <div className="border-b border-brand-border/40 bg-gradient-to-r from-brand-primary/8 via-brand-cream/50 to-brand-gold/10 px-5 py-4 sm:px-6">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="relative flex-1">
              ...
              <input
                ...
                autoComplete="off"
                spellCheck={false}
                className="h-12 w-full rounded-xl border-2 border-brand-border bg-white pl-11 pr-4 font-mono text-base font-bold tracking-wide text-brand-ink outline-none transition-all placeholder:font-sans placeholder:text-sm placeholder:font-normal placeholder:tracking-normal focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/15"
              />
            </div>
            <button
              type="submit"
              disabled={lookupLoading || !lookupCode.trim()}
              className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-brand-primary px-6 text-sm font-semibold text-white shadow-sm transition-all hover:bg-brand-primary-dark disabled:opacity-60"
            >
              {lookupLoading ? (
        ...
        <div className="p-5 sm:p-6">
          <AnimatePresence mode="wait">
            {lookupLoading ? (
              ...
              >
                <Loader2 className="h-5 w-5 animate-spin text-brand-primary" strokeWidth={2} aria-hidden />
                Fetching booking…
              </motion.div>
            ) : lookupBooking ? (
              <BookingLookupResult
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="flex items-center gap-2 text-sm font-bold text-brand-ink">
            <ClipboardList className="h-4 w-4 text-brand-primary" strokeWidth={2} aria-hidden />
            All reservations
